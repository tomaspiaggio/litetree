...
const INSTRUCTION_LINES = [
  ...
  "- Call get_context first, before anything else. If branchNameIsGenerated is true, the branch still has a throwaway name: make renaming it your very first action — infer a name from the user's request, rename the git branch (follow the repo's existing branch-naming convention, e.g. `fix/...` or `feat/...`), then call report_branch with that branch name AND a short, human displayName for the sidebar. The sidebar is narrow (~18 characters, and a PR number suffix eats into that), so the displayName must NOT just echo the branch name — drop the `fix/`/`feat/` prefix and any redundant words and write a punchy label a human would skim, e.g. branch `fix/pr-merged-vs-closed-state` → displayName `pr-merged fix`. Do this before starting the actual task, so the sidebar shows a meaningful name even if the user steps away while you work.",
