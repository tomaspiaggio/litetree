...
export interface WorktreeContext {
  ...
  hasPR: boolean
  baseBranch: string
  // True when the branch still has treemux's autogenerated throwaway name
  // (e.g. `deep-morning-lynx-fwu`). When set, the agent should rename the
  // branch to something descriptive and report it back via report_branch.
  branchNameIsGenerated: boolean
...
export function registerTools(server: McpServer, deps: ToolDeps): void {
  ...
  server.registerTool(
    "get_context",
    {
      ...
      description:
        "Get treemux's context for this worktree: branch name, worktree path, project name, current PR number (if any), the base branch to target for PRs, and whether the branch still has an autogenerated throwaway name (branchNameIsGenerated).",
      inputSchema: {},
    },
    async () => {
